...
- **工单自动分配**：新工单未手动指派时，按「未完成工单数最少」的客服自动指派（优先普通客服），可在系统设置中开关
- **系统设置页**（管理端）：系统名称、自动分配开关、各优先级 SLA 时限，运行时保存即时生效（`settings` 表）；另展示实时服务/后台前缀/短信/微信通道运行状态
- **列表实时与兜底**：工单列表页建立 WebSocket 连接（`ticket.all` 房间），新工单/更新实时提示；WS 掉线自动降级为 20s 轮询 `/tickets/changes`
