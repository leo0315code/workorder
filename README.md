...
- **登录体系（用户端/管理端分离）**
  ...
  - 手机号登录：验证码 5 分钟有效，未注册手机号自动创建客户账号；短信驱动/密钥在**系统设置页**运行时维护（demo 本地联调验证码直显，切阿里云/腾讯云只需填密钥签名模板，无需改 .env）
...
- **客服角色管理（RBAC 风格）**：
  - **角色模板**：在「角色管理」页维护客服角色（如 客服主管/普通客服/只读客服），每角色绑定一组模块权限（客户档案/产品/分类/快捷回复/模板/报表/导出/批量）
  - **分配角色**：用户管理页「客服角色」列下拉分配，菜单按角色模块渲染 + 后端中间件守卫
  - 预置 3 个角色（主管/普通/只读）可作起点；未分配角色 = 默认全模块
- **数据报表**：近 7/30/90 天每日新增趋势、客服处理排行（处理数/回复数/平均首次响应时长）、状态/优先级/分类分布、满意度；**一键导出 CSV**（当前时间范围，UTF-8 BOM 直开 Excel）
- **实时推送**：新回复 / 状态变更 / 新工单 / 新通知即时送达（WS 不可用自动降级：详情页 8s 轮询、列表页 20s 轮询）
- **WSS 支持（生产 HTTPS 必备）**：统一走 **Nginx/Apache 反向代理终结 TLS**——设置 `WS_PROXY_PATH=/ws`，页面按协议自动生成 `wss://域名/ws` 或 `ws://域名/ws`；GatewayWorker 保持明文 `ws://127.0.0.1:6001`（本地联调直连）
