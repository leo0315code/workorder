...
- 输入框 type=email → text，去掉 email 格式校验；占位符/标签同步更新
- 测试：+2（用户名登录成功 / 错误用户名失败），全量 314 用例 / 864 断言

## 2026-09-12 — v2.4.2：回复快捷键 + 编号复制 + 通知已读筛选

- **Enter 发送 / Shift+Enter 换行**：回复框快捷键（中文输入法组词不误触），发送按钮旁有提示
- **工单编号一键复制**：详情页面包屑旁复制按钮，复制后图标变 ✓
- **通知中心「已读」筛选**：全部/未读/已读 三个 tab（`read=1` 参数，UserNotification 补 `scopeRead`）

- 测试：+1（已读/未读筛选互斥），全量 312 用例 / 860 断言

## 2026-09-12 — v2.4.1：客户知识库入口 + 工单催办

**客户「帮助中心」（知识库自助入口）**
- 侧边栏新增「帮助中心」：分类筛选（带文章数）+ 关键字搜索 + 文章卡片列表（浏览数/更新时间）
- 仅展示已发布文章，草稿不可见；阅读页复用原有 `kb.show`

**工单催办（限频 24h）**
- 客户在工单详情页可点「催办」，通知负责人（未指派 → 全体客服/管理员）
- 每 24 小时限一次（基于操作日志 `urged` 记录），已催办显示状态徽标；已解决/关闭不可催
- 客服/非本人工单 403 拦截

- 测试：KbTest +2（仅已发布/分类与搜索过滤）、UrgeTest +6、MenuTest 菜单数 19

## 2026-09-12 — v2.4.0：粘贴截图发工单 + 全局操作审计页

**工单/回复支持 Ctrl+V 粘贴截图**
- 剪贴板图片自动并入 `attachments[]`（DataTransfer 合并进文件输入框，走原生表单提交，零后端改动）
- 创建工单与回复框双端生效，附件区提示"支持 Ctrl+V 粘贴截图"

**新增「操作审计」页（仅管理员，/console/ticket-logs）**
- 跨工单检索全部 `TicketLog`（创建/回复/备注/状态与字段变更），含操作人/动作/关键字/日期范围筛选
- 数据源与工单详情内时间线同表，无冗余存储；侧边栏「系统管理」新增入口（菜单 Seeder 幂等更新）
- 测试：TicketLogTest 6 例（权限/按人/按动作/按编号/按日期范围）；MenuTest 菜单数 17→18
